...
+(NSString *)convertNumberToMoneyString:(float)amount {
	return [NSString stringWithFormat:@"%d", (int)round(amount)];
}
...
+(UIImage *)pieChartWithItems:(NSArray *)itemList startDegree:(float)startDegree {
	...
	if(totalPieSize > 0) {
		...
		for (GraphObject *graphObj in itemList) {
			...
			percentStr = [self convertNumberToMoneyString:value];
	...
	for(int month=1; month<=12; month++) {
		...
		if(recordExists) {
			...
			if(month==displayMonth) {
				...
				if(type==1 || type==2)
					label = @"Value";
				todayAmount = [NSString stringWithFormat:@"%@: %@", label, [self convertNumberToMoneyString:asset_value]];
				...
				todayAmount2 = [NSString stringWithFormat:@"%@: %@", label, [self convertNumberToMoneyString:balance_owed]];
